...
export default function StrategyBox(props) {
  ...
  useEffect(() => {
    const validInputValue = isNaN(parseInt(inputValue, 10))
      ? 0
    ...
    if (productList.length && validInputValue && categoryRatio) {
      const updatedProducts = productList.map((product) => {
        ...
        let amount = (
          parseFloat(formattedValue.replace(/,/g, "")) / currentPrice
        ).toFixed(2);
        if (categoryName === "해외") {
          console.log("해외");
          amount = (
    ...
    <BoxContainer>
      ...
      <Body>
        {formattedProducts.map((product, index) => {
          const currentPrice = parseFloat(productPrices[product.code]) || 1;
          let amount = (
            parseFloat(product.formattedValue.replace(/,/g, "")) / currentPrice
          ).toFixed(2);
          if (categoryName === "해외") {
            amount = (
              parseFloat(product.formattedValue.replace(/,/g, "")) /
              currentPrice /
              1350
            ).toFixed(2);
          }
          ...
          let additionalValue =
            (product.additionalAmount * currentPrice).toFixed(0) || 1;
          if (categoryName === "해외") {
            console.log("additionalAmount (해외):", additionalAmount);
            console.log("currentPrice (해외):", currentPrice);
            additionalValue =
              (product.additionalAmount * currentPrice * 1350).toFixed(0) || 1;
          ...
          return (
            <Row key={index}>
              ...
              <StockContainer>
                <StockContent>{product.myStrategyRatio * 100}%</StockContent>
              </StockContainer>
              <StockContainer>
                <StockContent>{product.ratio * 100}%</StockContent>
              </StockContainer>
              <StockContainer>
                <StockContent>{product.formattedValue}원</StockContent>
              </StockContainer>
              <StockContainer>
                <StockContent>{amount}주</StockContent>
              </StockContainer>
              <StockContainer>
                <Stock_Content applyClicked={applyClicked}>
                  {additionalAmount}주
                </Stock_Content>
              </StockContainer>
              <StockContainer>
                <Stock_Content applyClicked={applyClicked}>
                  ...
                  {isNaN(additionalValue)
                    ? "0원"
                    : formatNumber(additionalValue) + "원"}
                </Stock_Content>
              </StockContainer>
            </Row>
          );
