        ...
        const fetchData = async () => {
            try {
                ...
                const newBalanceBankInfoData = [
                    {
                        label: '총 잔액',
                        amount: result.bankTotalAmount.toLocaleString() + '원',
                    },
                    {
                        label: '총 출금가능액',
                        amount:
                            result.bankWithdrawableAmount.toLocaleString() +
                            '원',
                    },
                ];
                ...
                const newSlidesSecData = result.securities
                    .map((elem) =>
                ...
                const newBalanceSecInfoData = [
                    {
                        label: '총 잔액',
                        amount:
                            result.securitiesTotalAmount.toLocaleString() +
                            '원',
                    },
                    {
                        label: '총 출금가능액',
                        amount:
                            result.securitiesTotalPurchaseAmount.toLocaleString() +
                            '원',
                    },
                ];
                ...
                const newSlidesHouseData = result.housing.map((elem) => {
                    ...
                    const currentBalance = `평가금액 ${formatCurrency(
                        evalAmount
                    )}원`;
                    const profitBalance = `(${sign}${formatCurrency(
                        profitAmount
                    )})`;
                ...
                const newBalanceHouseInfoData = [
                    {
                        label: '총 가치',
                        amount:
                            result.housingTotalAmount.toLocaleString() + '원',
                    },
                ];
                ...
                const newSlidesDebtData = result.debt.map((elem) => ({
                    ...
                    productName: `잔여 ${formatCurrency(
                        elem.remainAmount
                    )}원 (총 ${formatCurrency(elem.principalAmount)}원)`,
                    currentBalance: `월 상환액 ${formatCurrency(
                        elem.transAmount
                    )}`,
                }));
                ...
                const newBalanceDebtInfoData = [
                    {
                        label: '남은 총액',
                        amount: result.debtRemainAmount.toLocaleString() + '원',
                    },
                    {
                        label: '총 대출금',
                        amount: result.debtTotalAmount.toLocaleString() + '원',
                    },
                ];
    ...
    return (
        <StyledHomeContainer>
            ...
            <StyledHomeMainContent>
                <Sidebar />
                <StyledHomeContent
                    style={{ overflow: 'hidden', padding: '40px' }}
                >
                    <div
                        style={{
                            width: '910px',
                    ...
                    >
                        <StyledHeadText>
                            ...
                            >
                                ※ 자산 정보를 보내지 않을 경우, PB는 내 자산에
                                대한 정보를 확인할 수 없습니다.
                            </p>
                        </StyledHeadText>

                        <div style={{ display: 'flex', gap: '10px' }}>
                            <Button id="goback" onClick={onClickPrev}>
                                돌아가기
                            </Button>
                            <Button
                                id="mydata-api"
                                onClick={onClickMydataButton}
                            >
                                다음으로
                            </Button>
                        </div>
